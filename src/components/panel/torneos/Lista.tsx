...
export default function Lista() {
    ...
    useEffect(() => {
        async function carregarTornejos() {
            try {
                ...
                if (!response.ok) {
                    throw new Error("Error en carregar els tornejos");
                }
    ...
    return(
        <div className="w-full h-auto relative p-4 flex items-center max-md:place-content-center gap-4 flex-wrap">
            {tornejos.map((torneig) => (
                <div className="w-64 h-105 bg-card rounded-lg" key={torneig.id}>
                    <div className="w-full h-32 bg-card rounded-t-lg flex items-center  justify-center relative">
                        <img src={torneig.banner} alt={torneig.nombre} className="w-full h-full object-cover rounded-t-lg" />

                        <div className="absolute w-16 h-16 p-px rounded-lg overflow-hidden bg-card z-30  left-4 shadow-border shadow-sm -bottom-8">
                            <img src={torneig.logo} alt={torneig.nombre} className="w-full h-full object-cover" />
                        </div>
                    ...
                    <div className="w-full h-full mt-10 p-2 flex flex-col gap-y-1">
                        ...
                        <p className="text-lg font-bold text-neutral-titulos">{torneig.nombre}</p>
                        ...
                        <div className="w-full grid grid-cols-2 gap-x-2 place-items-center text-sm">
                            ...
                            <div className="w-full px-2">
                                ...
                                <p className="text-neutral-titulos">{formatearFechaCatalan(torneig.fecha_creacion)}</p>
                            </div>
                        </div>

                        <div className="w-full h-0.5 bg-muted/40 rounded my-3">&nbsp;</div>

                        <a href={`/panell?torneoID=${torneig.id}`} className="mx-auto w-max h-max px-4 py-2 bg-background/80 border-primary rounded-lg border text-secondary hover:bg-background/60 duration-300">
                            Administrar
                        </a>
            ...
            <div className="w-64 h-105 bg-card rounded-lg relative">
                    <div className="w-full h-32 bg-primary/30 rounded-t-lg flex items-center  justify-center relative">
                        <svg xmlns="http://www.w3.org/2000/svg" className="fill-secondary w-16 h-16" viewBox="0 -960 960 960">
                    ...
                    <div className="w-full h-full mt-10 p-2 flex flex-col items-center relative gap-y-5">
                            ...
                            <p className="text-lg font-bold text-neutral-titulos">Nou Torneig</p>
