...
function obtenerNombre(usuario: UsuarioPaso) {
    return [
        usuario.nombre,
        usuario.apellido1,
        usuario.apellido2,
...
function obtenerIniciales(usuario: UsuarioPaso) {
    const partes = [
        usuario.nombre,
        usuario.apellido1,
        usuario.apellido2,
...
function obtenerOrigen(valor: string | null) {
...
export default function PasoUsuario({
    ...
    const origen = creando
        ? "Manual"
        : obtenerOrigen(usuario.origen_permisos);
    ...
    const datos = [
        ...
        {
            etiqueta: "Primer cognom",
            valor: usuario.apellido1,
        },
        {
            etiqueta: "Segon cognom",
            valor: usuario.apellido2,
        },
        {
        ...
        >
            <header className="border-b border-border pb-5">
                ...
                <p className="mt-2 max-w-2xl text-sm leading-6">
                    {creando
                        ? "Comprova que has seleccionat el compte correcte abans de configurar els seus accessos."
                        : "Consulta el compte al qual pertany aquesta configuració de permisos."}
                </p>
            </header>
                ...
                <div className="flex min-w-0 items-center gap-4">
                    <div
                        ...
                        className="
                            ...
                            border-border bg-card text-xl font-semibold
                        "
                    >
            ...
            <dl className="grid gap-x-6 gap-y-5 sm:grid-cols-2">
                {datos.map(({ etiqueta, valor }) => (
                    <div key={etiqueta} className="min-w-0">
                        ...
                        <dd
                            className="
                                min-h-11 wrap-break-words rounded-lg
                                border border-border bg-card
                                px-3.5 py-3 text-sm
                            "
                        >
                            {valor?.trim() || "Sense informació"}
                        </dd>
                    </div>
                ))}
            </dl>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
                ...
                <div className="rounded-xl border border-border p-4">
                    ...
                    <p className="mt-2 text-sm leading-6">
                        {modo === "ver"
                            ? "Als passos següents pots consultar els accessos i el rol assignat a cada torneig."
                            : "Al següent pas podràs assignar els tornejos i els rols. El rol general serà el més baix dels assignats."}
                    </p>
                </div>
            </div>

            {creando && usuario.activa !== true && (
                <div
                    role="alert"
                    className="
                ...
                >
                    ...
                    <div>
                        ...
                        <p className="mt-1 text-sm leading-6">
                            Activa aquest compte des de la gestió d'usuaris
                            abans de concedir-li accés al panell.
                        </p>
                    </div>
                </div>
            )}
