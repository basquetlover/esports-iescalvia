...
export type ResultadoEmailSinPlaza = {
...
async function cargarDatosEmail(formularioID: string): Promise<DatosEmail> {
    ...
    if (equipo.plaza_estado?.trim().toUpperCase() !== "SIN_PLAZA") {
        throw new Error("El correu només es pot enviar quan la plaça està SIN_PLAZA.");
    }
...
function crearAsunto(datos: DatosEmail): string {
    return `Plaça no assignada · ${datos.equipo.nombre?.trim() || "Equip"} | ${datos.torneo.nombre?.trim() || "Esports IES Calvià"}`;
}
...
<head>
    ...
    <title>Plaça no assignada</title>
...
<table width="100%" cellspacing="0" cellpadding="0" role="none" style="border-spacing:0;padding:0;Margin:0;width:100%;height:100%"><tbody><tr><td valign="top" align="center" style="padding:0;Margin:0">
    ...
    <table cellspacing="0" cellpadding="0" align="center" width="600" class="es-header" role="none" style="border-spacing:0;width:600px;max-width:600px"><tbody>
        <tr><td bgcolor="#0b1c30" align="center" style="padding:24px 20px 18px;Margin:0;background-color:#0b1c30">
            ...
            <h4 style="Margin:8px 0 0 0;font-size:22px;font-weight:normal;line-height:30px;color:#ffffff;text-align:center">Plaça no assignada</h4>
        </td></tr>
        ...
        <tr><td bgcolor="#0b1c30" align="left" style="padding:0 20px 20px;Margin:0;background-color:#0b1c30">
            <table width="100%" cellspacing="0" cellpadding="0" role="presentation" style="border-spacing:0"><tbody><tr>
                <td align="center" bgcolor="#213145" style="padding:18px;Margin:0;background-color:#213145;border:1px solid #3e4947;border-radius:12px">
                    <h3 style="Margin:0;font-family:Verdana,Geneva,sans-serif;font-size:25px;font-weight:normal;line-height:32px;color:#ffffff;text-align:center">L'equip no ha obtingut plaça</h3>
                    <p style="Margin:12px 0 0 0;font-family:Verdana,Geneva,sans-serif;line-height:21px;color:#d3e4fe;font-size:14px;text-align:center">En aquesta edició no s'ha pogut assignar una plaça a l'equip. La inscripció continuarà disponible per consultar-ne l'estat.</p>
                </td>
            </tr></tbody></table>
        </td></tr>
    </tbody></table>
...
export async function enviarNotificacionSinPlaza(
    formularioID: string,
): Promise<ResultadoEmailSinPlaza> {
    ...
    resultados.forEach((resultado, indice) => {
        ...
        if (resultado.status === "fulfilled") {
            enviados += 1;
            console.info(`[EMAIL] SIN_PLAZA enviado a ${destinatario?.tipo ?? "DESCONOCIDO"} (${destinatario?.email ?? "sin-email"}).`);
        } else {
            fallidos += 1;
            console.error(`[EMAIL] Error enviando SIN_PLAZA a ${destinatario?.tipo ?? "DESCONOCIDO"} (${destinatario?.email ?? "sin-email"}):`, resultado.reason);
        }
    });
